...
class Efficiency:
    ...
    def z_efficiency(self):
        ...
        eff_rgi, Z_rgi = [], []
        eff_, Z_ = [], []
        z = np.arange(np.min(self.metadata['z']), np.max(self.metadata['z']), 0.01)
        ...
        for i in range(0,len(z)-1):
            ...
            md_rg = self.mask_to_apply(self.metadata, var=['z', 'z', 'n_i_band', 'sel'], op=[op.ge, op.lt, op.eq, op.eq], lim=[zmin, zmax, 0, 1])
            md_rgi = self.mask_to_apply(self.metadata, var=['z', 'z', 'n_i_band', 'sel'], op=[op.ge, op.lt, op.ne, op.eq], lim=[zmin, zmax, 0, 1])
    
            #sans distinction
            md_ = self.mask_to_apply(self.metadata, var=['z', 'z', 'sel'], op=[op.ge, op.lt, op.eq], lim=[zmin, zmax, 1])
            md_tot = self.mask_to_apply(self.metadata, var=['z', 'z'], op=[op.ge, op.lt], lim=[zmin, zmax])
